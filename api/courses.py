...
async def get_supervision_academica(
    ...
    F-FIX-SUPERVISION-PERMISO (2026-08-19): antes usaba require_staff, que
    EXCLUYE a COORDINADOR (solo admin/superadmin/mae/cpd/cobranza) -- el
    destinatario principal de esta vista es justamente el coordinador, asi
    que con require_staff nunca podia entrar (403 antes de llegar siquiera
    al 422 de colision de rutas que tambien tenia este endpoint). Chequeo
    inline en vez de reusar require_encargado_curso porque ese excluye a
    MAE (que si debe verla) e incluye a ENCARGADO_CURSO (que no la
    necesita: ya ve su unico programa en Programas).
    """
    if not isinstance(current_user, User) or current_user.rol not in (
        UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.CPD, UserRole.MAE, UserRole.COORDINADOR,
    ):
        raise HTTPException(
            status_code=403,
            detail="Acceso restringido a coordinadores y personal de gestión.",
        )
